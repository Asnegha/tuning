...
def main(
    ...
    def inference(input_prompt, generator):
        ...
        file = open('llama2_English_tuned_xsquad_prompt2f_withoutgate.txt', 'a') 
    ...
    for lang in ['en']:
        ...
        for inst in dataset:
            ...
            match = re.search(r"\['answer':\s*(.+?)\]", outputs)
            print(match)
            if match:
                res = match.group(1)
                print(res)
            ...
            file = open('llama2_English_tuned_xsquad_withoutgate.txt', 'a') 
    ...
    df = pd.DataFrame(l)
    df = df.T
    df.to_csv("llama2_English_tuned_xsquad_withoutgate.csv")
